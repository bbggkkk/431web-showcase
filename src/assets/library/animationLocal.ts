...
const getAnimationProps = (animation:animation) => {
    ...
    return [...new Set(ani.map(item => Object.keys(item as any)).flat())];
}

const fillUndefinedProp = (animation:any, baseKeyframe:Array<number>, keyframeKeys:Array<string>, props:Array<string>) => {
    ...
    postMessage(data:any){
        setTimeout(() => {
            ...
            if(idx === undefined){
                ...
                this.onmessage(animationKeyframe);
            }else{
                if(idx === 0){
                    this.onmessage({data:animation[0]});
                }else if(idx === length){
                    this.onmessage({data:animation[100]});
                }else{
                    this.onmessage({data:fillOneProp(animation, animationKeyframe, keyframeKeys, baseKeyframe, props, idx)});
                }
            }
            // close();
        },0);
