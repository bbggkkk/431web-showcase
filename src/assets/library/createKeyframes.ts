...
export const gotoAndStop = (element:any, animation:Array<animationValue>, frame:number) => {
    let style = animation[frame];
    Object.keys(style).forEach(async (item:any) => {
...
export const createKeyframes = (animation:animation, length:number|Function, option?:option):Array<any> => {
    ...
    const updator = updateAnimation(animation, length, (fnKeys as any), (colorKeys as any));
    const getKeyframor = getKeyframe(animation, length, (fnKeys as any), (colorKeys as any));
...
export const colorRGBLize = ($color:any):string => {
    let str = $color.replace(colorReg, (match:any, p1:any) => {
        if(match.substring(0,1) === '#'){
            let color = '';
            switch (match.length){
                case 4 :
                    const cs = match.split('').splice(1,4);
                    color = (cs.reduce((acc:any,item:any) => {
                        acc+=(item+item);
                        return acc;
                    },''))+'ff';
                    break;
...
export const findSpcKeys = (animation:animation):option => {
    ...
    const result = Object.keys(animation).reduce((acc:any, item:string) => {
